...
def is_responding(name):
    ...
    r = subprocess.check_output('tasklist /FI "IMAGENAME eq %s" /FI "STATUS eq running"' % name).strip().split('\n')

    #see if the process runnin is parameter one (return true)
    #or the file sent error mensage (return false)

    if r == ['']:  # prevents error when call returns 'INFO: No tasks running with specified criteria'
        return False
...
def monitor_once():
    ...
    sc_crashed = not is_responding("starcraft.exe") or not is_responding("Chaoslauncher.exe")

    if sc_crashed or big_error:
        print 'StarCraft has crashed or got lost writing error file.'

        both_closed = not is_responding("starcraft.exe") and not is_responding("Chaoslauncher.exe")
        #should restart. Kill both starcraft and chaoslaucher

        while not both_closed:
            # /F means to forcefully terminate process /T is to kill child processes too

            print 'Will attempt to kill StarCraft and Chaoslauncher processes.'
