...
\begin{abstract}
Real time strategy games are complex scenarios where multiple agents must be coordinated in a dynamic, partially observable environment. In this work, we model coordination of these agents as a task allocation problem and employ a task allocation algorithm based on swarm intelligence. To adjust the parameters of the task allocation algorithm we use a genetic algorithm. This approach is tested in the popular video game StarCraft: BroodWar. As the fitness evaluation on the genetic algorithm depends on match results, which are time-consuming, we employ a fitness estimation method to eliminate the need to evaluate all individuals, accelerating the convergence of this algorithm. Our results show that the genetic algorithm enhances the performance of the task allocation algorithm. The proposed approach is able to defeat StarCraft's native AI in most matches. Moreover, fitness estimation proved to be [useful/unreliable] in our experiments [if unreliable: because match results aren't deterministic]. 
\end{abstract}
